...
| RF02 | O sistema deve permitir cadastrar uma matrícula única para cada aluno. | Alta |
| RF03 | O sistema deve permitir consultar os alunos cadastrados. | Alta |
| RF04 | O sistema deve permitir pesquisar alunos por nome ou matrícula. | Média |
| RF05 | O sistema deve permitir visualizar os dados cadastrais completos de um aluno. | Alta |
| RF06 | O sistema deve permitir editar os dados cadastrais de um aluno. | Alta |
| RF07 | O sistema deve permitir excluir um aluno, respeitando a integridade dos registros acadêmicos relacionados. | Média |
| RF08 | O sistema deve permitir consultar o histórico acadêmico de um aluno. | Média |
...
| RF10 | O sistema deve permitir informar o período letivo da turma. | Alta |
| RF11 | O sistema deve permitir consultar as turmas cadastradas. | Alta |
...
| RF14 | O sistema deve permitir matricular alunos em uma turma. | Alta |
...
| RF18 | O sistema deve permitir informar o nome, código e demais dados necessários da disciplina. | Alta |
| RF19 | O sistema deve permitir consultar as disciplinas cadastradas. | Alta |
| RF20 | O sistema deve permitir editar os dados de uma disciplina. | Média |
| RF21 | O sistema deve permitir excluir uma disciplina, respeitando a integridade dos registros acadêmicos relacionados. | Média |
...
| RF28 | O sistema deve permitir alterar uma nota previamente lançada. | Alta |
| RF29 | O sistema deve permitir consultar as notas de um aluno. | Alta |
| RF30 | O sistema deve permitir consultar as notas de todos os alunos de uma turma ou disciplina. | Alta |
| RF31 | O sistema deve calcular automaticamente a média ponderada do aluno. | Alta |
| RF32 | O sistema deve atualizar automaticamente a média após alterações nas notas. | Alta |
| RF33 | O sistema deve impedir o lançamento de notas fora da escala de avaliação definida. | Alta |
...
| RF35 | O sistema deve permitir registrar faltas por aula ou data. | Alta |
| RF36 | O sistema deve permitir consultar o histórico de frequência de um aluno. | Alta |
...
| RF39 | O sistema deve permitir consultar a frequência de todos os alunos de uma turma ou disciplina. | Alta |
| RF40 | O sistema deve identificar alunos próximos ou abaixo do limite mínimo de frequência. | Alta |
...
| RF45 | O sistema deve identificar alunos em situação de recuperação, caso essa modalidade seja adotada. | Média |
...
Os requisitos relacionados às regras de aprovação, reprovação,
recuperação e frequência deverão utilizar os critérios definidos
para o projeto acadêmico.

Os requisitos do módulo preditivo deverão ser refinados após a
definição das variáveis utilizadas pelo modelo de Machine Learning,
da base de dados disponível e da estratégia de classificação.